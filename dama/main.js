...
window.addEventListener('load', async () => {
    ...
    socket.on('luckySpinResult', (data) => {
        ...
        } else { if (typeof ui.showCustomAlert === 'function') ui.showCustomAlert(data.message, "عذراً"); }
    });
        ...
        if (data && data.popValue) {
            ...
            if (window.POPULARITY_ITEMS) {
                ...
                if (giftObj) {
                    ...
                    } else { giftImageHtml = `<img src="${giftObj.imagePath}" style="${style}">`; }
                }
            }

            const celebrationOverlay = document.createElement('div');
            celebrationOverlay.style.cssText = `position: fixed; top: 0; left: 0; width: 100vw; height: 100dvh; background: rgba(0,0,0,0.85); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); z-index: 999999999; display: flex; flex-direction: column; align-items: center; justify-content: center; animation: fadeInOverlay 0.4s ease-out;`;
            ...
            celebrationOverlay.innerHTML = `
                ...
                <div style="position: relative; overflow: hidden; background: linear-gradient(135deg, rgba(30,32,40,0.95), rgba(15,18,25,0.95)); border: 2px solid #ffd700; border-radius: 28px; padding: 40px 30px; text-align: center; box-shadow: 0 0 40px rgba(255,215,0,0.3); width: 85%; max-width: 340px; animation: popInModal 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
                    ...
                    <div style="margin: 25px 0; position: relative; z-index: 2;">${giftImageHtml}</div>
            ...
            setTimeout(() => { celebrationOverlay.style.opacity = '0'; celebrationOverlay.style.transition = 'opacity 0.5s ease'; setTimeout(() => celebrationOverlay.remove(), 500); }, 4500);
        }
    });
...
ui.onClick('start-white-btn', () => { gameState.playerColor = 'white'; if(typeof ui.initBoard === 'function') ui.initBoard(); if(typeof window.closeAppModal === 'function') window.closeAppModal('new-game-modal'); });
ui.onClick('start-black-btn', () => { gameState.playerColor = 'black'; if(typeof ui.initBoard === 'function') ui.initBoard(); if(typeof window.closeAppModal === 'function') window.closeAppModal('new-game-modal'); });
ui.onClick('new-game-modal', e => { if (e.target.id === 'new-game-modal' && typeof window.closeAppModal === 'function') window.closeAppModal('new-game-modal'); });
ui.onClick('cancel-new-game-btn', () => { if(typeof window.closeAppModal === 'function') window.closeAppModal('new-game-modal'); });
ui.onClick('settings-btn', e => { e.stopPropagation(); if(typeof window.openAppModal === 'function') window.openAppModal('settings-overlay'); });
ui.onClick('save-settings-btn', () => { saveGameState(); if(typeof window.closeAppModal === 'function') window.closeAppModal('settings-overlay'); });
ui.onClick('settings-overlay', e => { if (e.target.id === 'settings-overlay' && typeof window.closeAppModal === 'function') window.closeAppModal('settings-overlay'); });
...
ui.onClick('login-submit-btn', () => {
    ...
    if (!name) { if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert(t('enter_name')); return; }
...
ui.onClick('add-friend-btn', () => {
    ...
    if (!fId || fId === gameState.userProfile.id || (gameState.userProfile.friends && gameState.userProfile.friends.includes(fId))) {
        if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert(t('invalid_id')); return;
    }
...
document.getElementById('avatar-upload-input')?.addEventListener('change', e => {
    ...
    if (!file || !file.type.startsWith('image/') || file.size > 800 * 1024) {
        if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert(t('img_large')); return;
    }
    const reader = new FileReader();
    reader.onload = ev => {
        gameState.userProfile.avatar = ev.target.result; gameState.userProfile.isCustomAvatar = true; 
        if(typeof ui.updateProfileUI === 'function') ui.updateProfileUI();
        try { localStorage.setItem('hub_user_profile', JSON.stringify(gameState.userProfile)); } catch(err) { if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert("Storage limit exceeded."); }
    };
...
ui.onClick('switch-account-btn', () => { if(typeof window.closeAppModal === 'function') window.closeAppModal('profile-modal'); if(typeof window.openAppModal === 'function') window.openAppModal('login-modal'); });
...
ui.onClick('spin-free-btn', () => {
    ...
    } else { if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert(t('server_disconnected') || "يرجى الاتصال بالإنترنت أولاً للعب عجلة الحظ!"); }
});

ui.onClick('spin-paid-btn', () => {
    if (window.isSpinning) return;
    if (gameState.userProfile.tokens < 200) { if(typeof ui.showCustomAlert === 'function') ui.showCustomAlert("رصيدك غير كافٍ للفة الإضافية (مطلوب 200 🪙)", "عذراً"); return; }
    if (socket && socket.connected) {
        if(typeof ui.showCustomAlert === 'function') {
...
if (onlineBtn) {
    onlineBtn.addEventListener('click', (e) => {
        ...
        if (window.myCurrentRoomId || gameState.myCurrentRoomId) {
            if (socketManager) { socket.emit('leaveRoom', { roomID: window.myCurrentRoomId || gameState.myCurrentRoomId }); window.myCurrentRoomId = null; gameState.myCurrentRoomId = null; }
        }

        const mmModal = document.getElementById('matchmaking-modal');
        if (mmModal) {
            mmModal.style.display = 'flex';
            if (gameState.modalStack && !gameState.modalStack.includes('matchmaking-modal')) { gameState.modalStack.push('matchmaking-modal'); history.pushState({ modalOpen: 'matchmaking-modal' }, ''); }
        }
        ...
        gameState.mmInterval = setInterval(() => {
            ...
            let m = String(Math.floor(gameState.mmTimeLeft / 60)).padStart(2, '0'); let s = String(gameState.mmTimeLeft % 60).padStart(2, '0');
...
if (cancelMmBtn) {
    cancelMmBtn.addEventListener('click', (e) => {
        ...
        const mmModal = document.getElementById('matchmaking-modal');
        if (mmModal) { mmModal.style.display = 'none'; if (gameState.modalStack) gameState.modalStack = gameState.modalStack.filter(id => id !== 'matchmaking-modal'); }
        if (socket && socket.connected) { socket.emit('leaveMatchmakingPool'); }
    });
}

ui.onClick('room-portal-btn', () => { if(typeof window.openAppModal === 'function') window.openAppModal('online-modal'); });
ui.onClick('online-close-btn', () => { if(typeof window.closeAppModal === 'function') window.closeAppModal('online-modal'); });
