...
window.renderDynamicLeaderboardUI = function(playersList, tabType) {
    ...
    podiumOrder.forEach(item => {
        ...
        const card = document.createElement('div'); card.className = `lb-podium-card rank-${item.rank}`;
        ...
        card.innerHTML = `
            ...
            <div class="lb-podium-score score-${item.rank}">${getFormattedLeaderboardScore(player, tabType)}</div>
        `;
    ...
    for (let i = 3; i < playersList.length; i++) {
        if(window.createLbItemHTML) listContainer.appendChild(window.createLbItemHTML(i + 1, playersList[i], tabType));
    }
};
...
window.switchLbTab = function(tabId) {
    ...
    if (tabId === 'wins' && window.lastFetchedWinsData) { window.renderDynamicLeaderboardUI(window.lastFetchedWinsData, 'wins'); } else if (tabId === 'xp' && window.lastFetchedXpData) { window.renderDynamicLeaderboardUI(window.lastFetchedXpData, 'xp'); }
};
