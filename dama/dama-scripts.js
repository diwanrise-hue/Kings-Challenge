...
// ==========================================
// 🎫 إدارة قسائم الخصم والحقيبة
// ==========================================
window.switchThemeGridTabCategory = function(category) {
    const tabs = ['bg', 'frames', 'pieces', 'vouchers', 'gifts'];
    tabs.forEach(tab => {
    ...
    if (activeBtn) activeBtn.classList.add('active');
    if (activeSec) {
        activeSec.style.display = category === 'vouchers' ? 'flex' : 'grid';
        if (category === 'vouchers') window.renderUserVouchersInBackpack();
    }
};

window.renderUserVouchersInBackpack = function() {
    const container = document.getElementById('theme-grid-section-vouchers');
    if (!container) return;
    container.innerHTML = '';
    ...
    const vouchers = profile.discountTickets || [];

    if (vouchers.length === 0) {
        container.innerHTML = '<p style="color: #a1a1aa; text-align: center; width: 100%; padding: 20px;">لا تمتلك أي قسائم خصم حالياً. اربحها من خلال تصدر لوحة الشرف! 🏆</p>';
        return;
    }

    vouchers.forEach((v) => {
        const card = document.createElement('div');
        card.style.cssText = "background: rgba(212,175,55,0.1); border: 1px solid #d4af37; border-radius: 14px; padding: 12px; display: flex; justify-content: space-between; align-items: center;";
        card.innerHTML = `
            <div>
                <div style="color: #ffd700; font-weight: bold; font-size: 14px;">🎫 ${v.title || ('خصم ' + v.rate + '%')}</div>
                <div style="color: #a1a1aa; font-size: 11px; margin-top: 2px;">تستخدم لخصم السعر عند الشراء من المتجر</div>
            </div>
            <div style="background: #d4af37; color: #1a1a24; padding: 4px 10px; border-radius: 8px; font-weight: bold; font-size: 12px;">متاحة</div>
        `;
        container.appendChild(card);
    });
};

window.toggleStoreVoucherDropdown = function() {
    const list = document.getElementById('store-voucher-dropdown-list');
    if (list) {
        list.style.display = list.style.display === 'block' ? 'none' : 'block';
    }
};

window.updateStoreVouchersUI = function() {
    const container = document.getElementById('store-voucher-menu-container');
    const list = document.getElementById('store-voucher-dropdown-list');
    ...
    if (vouchers.length === 0) {
        ...
        vouchers.forEach((v) => {
            ...
            item.style.cssText = "padding: 6px 8px; border-bottom: 1px solid rgba(255,255,255,0.05); cursor: pointer; font-size: 12px; color: #fff;";
            item.innerHTML = `🎫 ${v.title || ('خصم ' + v.rate + '%')}`;
            item.onclick = () => {
                window.activeStoreDiscountRate = v.rate; 
                alert(`✅ تم تفعيل قسيمة ${v.rate}% على عملية الشراء القادمة!`);
