...
// وضعنا كود الـ Worker كنص، هذه الطريقة تضمن أن الكود موجود دائماً ولا نحتاج لانتظار تحميل ملف خارجي
const workerCode = `
const AI_LEVELS = {
...
export const gameAI = {
    ...
    getWorkerInstance() {
        if (!this.workerInstance) {
            try {
                // الطريقة الآمنة لإنشاء Blob في متصفحات الهواتف
                const blob = new Blob([workerCode], { type: 'application/javascript' });
                const blobURL = window.URL.createObjectURL(blob);
                this.workerInstance = new Worker(blobURL);
            } catch (error) {
                console.warn("⚠️ متصفح الهاتف يمنع إنشاء Worker، سيتم استخدام البديل...");
                this.workerInstance = null; // سيجبر الكود على استخدام البديل فوراً
            }
        }
        ...
        return new Promise(async (resolve) => {
            ...
            try {
                if (window.Worker) {
                    ...
                    // إذا فشل المتصفح في إنشاء الـ Worker (أعطانا null)، لا ننتظر أبداً بل ننتقل للبديل فوراً!
                    if (!worker) {
                        return runFallback();
                    }

                    clearTimeout(this.workerFallbackTimer);
                    // قللنا المهلة لـ 1 ثانية فقط! (1000 ملي ثانية) 
                    // هكذا إذا كان المتصفح بطيئاً أو محظوراً بصمت، لن تلاحظ أي تأخير مزعج!
                    this.workerFallbackTimer = setTimeout(() => {
                        console.warn("⏱️ تحويل للبديل لبطء استجابة الجهاز...");
                        if (this.workerInstance) {
