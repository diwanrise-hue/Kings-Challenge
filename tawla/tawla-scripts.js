/**
 * tawla-scripts.js
 * المساعد العام للتنسيق والمزامنة وإدارة ساحة التحديات ونظام الرتب للعبة الطاولة
...
window.switchTawlaGameStoreTab = function(category) {
    const tabs = ['bg', 'fr', 'pc', 'offers', 'pop'];
    tabs.forEach(tab => {
        const btn = document.getElementById('tawla-tab-' + tab);
        let secId = 'store-section-' + (tab === 'fr' ? 'frames' : tab === 'pc' ? 'pieces' : tab === 'pop' ? 'popularity' : tab);
        const sec = document.getElementById(secId);
        if (btn) btn.classList.remove('active');
        if (sec) sec.style.display = 'none';
    });
    const activeBtn = document.getElementById('tawla-tab-' + category);
    ...
    if (activeSec) { activeSec.style.display = 'grid'; if (category === 'pop') window.renderTawlaPopularityStore(); }
};

window.renderTawlaPopularityStore = function() {
...
window.exitTawlaGame = function() { if (window.parent && window.parent !== window) window.parent.postMessage({ type: 'EXIT_GAME' }, '*'); };
...
function startSeasonCountdown() {
    ...
    function updateTimer() {
        const now = Date.now();
        const cycleLength = 11 * 60 * 1000; 
        const rewardTime = 10 * 60 * 1000;  
...
(function() {
    ...
    document.addEventListener('click', function(event) {
        ...
        const isGamePiece = event.target.closest('.piece, .cell, #tawla-board');
