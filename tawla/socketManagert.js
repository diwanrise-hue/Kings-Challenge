/**
 * socketManagert.js
 * النسخة المتطورة والمحصنة أمنياً للعبة الطاولة 🛡️
 */

import { gameState } from './gameStatet.js'; 
import { startOnlineHintSystem, restoreOfflineHintSystem } from './maint.js';
import { ui } from './uiControllert.js';
import { gameEngine } from './gameEnginet.js';

export const socket = io('https://diwanrise-dama-game-diwan.hf.space/tawla', { 
...
window.openCreateRoomModal = function() {
    ...
    if (vipContainer) {
        const currentVip = parseInt(profile.vipLevel) || 0;
...
window.renderRoomsList = function() {
    ...
    if (myRoom && window.currentRoomTab === 'play') {
        ...
        const avatarEl = document.getElementById('my-waiting-avatar');
        if (avatarEl && avatarEl.src !== avatarSrc) avatarEl.src = avatarSrc;
        
        const frameEl = document.getElementById('my-waiting-frame');
        let hostFrame = myRoom.equippedProfileFrame || myRoom.equippedFr; 
        const frameHTML = (hostFrame && miniFramesDB[hostFrame]) ? `<img src="${miniFramesDB[hostFrame]}" style="width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 2px 3px rgba(0,0,0,0.6)); transform: translateZ(0); will-change: transform;">` : '';
        if (frameEl && frameEl.innerHTML !== frameHTML) frameEl.innerHTML = frameHTML;

        const vipEl = document.getElementById('my-waiting-vip');
        const vipHTML = (myRoom.hostVipLevel && parseInt(myRoom.hostVipLevel) > 0) ? `<img src="Media/VIP/vip${myRoom.hostVipLevel}.webp" style="width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.8)); transform: translateZ(0); will-change: transform;">` : '';
        if (vipEl && vipEl.innerHTML !== vipHTML) vipEl.innerHTML = vipHTML;

        let displayName = myRoom.customTitle ? ("👑 " + myRoom.customTitle) : myRoom.hostName;
        const titleEl = document.getElementById('my-waiting-title');
        const targetTitle = myRoom.customTitle ? displayName : 'بانتظار الخصم...';
    ...
    const playRooms = roomsToRender.filter(r => !r.isFull);
    const spectateRooms = roomsToRender.filter(r => r.isFull);

    const updateContainer = (container, roomList, emptyMsg) => {
        if (!container) return;

        if (roomList.length === 0) {
            const currentEmpty = container.querySelector('.empty-rooms-msg');
            if (!currentEmpty) {
                container.innerHTML = `<p class="empty-rooms-msg" style="color: #a1a1aa; font-size: 13px; text-align: center; margin-top: 20px;">${emptyMsg}</p>`;
            }
            return;
        }
        ...
        roomList.forEach((r, index) => {
            ...
            roomEl.onmouseleave = () => roomEl.style.transform = 'translateZ(0) scale(1)';
            ...
            const getAvatarUI = (avatar, frame, vip) => {
                ...
                let frameHTML = frame && miniFramesDB[frame] ? `<img src="${miniFramesDB[frame]}" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) translateZ(0); width: 140%; height: 140%; z-index: 3; pointer-events: none; object-fit: contain; filter: drop-shadow(0 2px 3px rgba(0,0,0,0.6)); will-change: transform;">` : '';
                let vipHTML = vip && parseInt(vip) > 0 ? `<img src="Media/VIP/vip${vip}.webp" onerror="this.style.display='none';" style="position: absolute; top: -10px; right: -10px; width: 24px; height: 32px; object-fit: contain; z-index: 50; pointer-events: none; filter: drop-shadow(0 2px 4px rgba(0,0,0,0.8)); transform: translateZ(0); will-change: transform;">` : '';
            ...
            if (r.isFull) {
                ...
                const avatarsHTML = `
                    <div style="display: flex; align-items: center; gap: 12px; margin-left: 0; transform: translateZ(0); flex-shrink: 0;">
                        ...
                        <div style="width: 2px; height: 46px; background: linear-gradient(to bottom, transparent, #ffd700, transparent); border-radius: 2px;"></div>
    ...
    updateContainer(playListContainer, playRooms, "لا توجد غرف متاحة تطابق بحثك.");
    updateContainer(spectateListContainer, spectateRooms, "لا توجد مباريات جارية للمراهنة عليها حالياً.");
};
...
export const socketManager = {
    ...
    hidePingTimer: null, 
     ...
     _showToast(msg) {
        ...
        if (toast) {
            let cleanMsg = msg.replace('✨', '').trim();
            toast.innerHTML = `<span style="font-size: 18px; filter: drop-shadow(0 0 5px rgba(0,210,255,0.6)); flex-shrink: 0;">✨</span> <span style="flex: 1; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${cleanMsg}</span>`;
            toast.className = '';
    ...
    _initRealPingIndicator() {
        ...
        if (!pingEl) {
            ...
            pingEl.style.cssText = `
                position: absolute; bottom: -1px; left: calc(50% + 118px); 
                background: transparent; color: #66bb6a; 
                font-family: monospace; font-size: 11px; font-weight: 700; 
                z-index: 800; display: flex; align-items: center; justify-content: flex-start; gap: 4px;
            ...
            if (turnBoxWrapper) { turnBoxWrapper.style.position = 'relative'; turnBoxWrapper.appendChild(pingEl); } 
            else { document.body.appendChild(pingEl); }
        }
    ...
    _ensureUserProfile() {
        const isValidId = (id) => {
            ...
            return id.startsWith('GUEST-') || id.startsWith('USER-') || id.startsWith('FB-') || id.startsWith('TAWLA-');
        };
