/**
 * gameAIt.js
 * نظام الذكاء الاصطناعي الخاص بالكمبيوتر للعبة الطاولة
 */
...
const AI_LEVELS = {
    ...
    7: { id: 7, depth: 8, randomChance: 0.00, maxTime: 15000, name: "الزعيم (عمق 7)" }, 
    8: { id: 8, depth: 9, randomChance: 0.00, maxTime: 20000, name: "المصباح السحري" } 
};
...
const ZOBRIST = {
    pieces: {
        'white': new Array(64),
        'black': new Array(64),
        'white-tawla': new Array(64),
        'black-tawla': new Array(64)
    },
    blackTurn: 0n,
    ...
    init() {
        ...
        for (let i = 0; i < 64; i++) {
            ...
            this.pieces['white-tawla'][i] = nextRand64();
            this.pieces['black-tawla'][i] = nextRand64();
        }
...
export const gameAI = {
    ...
    generateBoardHash(board, currentTurn) {
        ...
        for (let r = 0; r < 8; r++) {
            for (let c = 0; c < 8; c++) {
                ...
                else if (p === 'white-tawla') hash += 'X';
                else if (p === 'black-tawla') hash += 'Y';
            }
        }
        return hash;
    },
    ...
    applyMoveToBoard(board, movePath, color, pieceDir) {
        ...
        if (endR === promoRow && !piece.includes('tawla')) {
            piece += '-tawla';
        }
    ...
    makeMove(board, movePath, color, pieceDir, currentHash) {
        ...
        const isPromoted = (endR === promoRow && !piece.includes('tawla'));
        const finalPiece = isPromoted ? (piece + '-tawla') : piece;
    ...
    evaluateBoard(board, aiColor, pieceDirection, levelNum) {
            ...
            for (let c = 0; c < 8; c++) {
                ...
                if (p) {
                    ...
                    if (isMine) { 
                        ...
                        oppPieces++; 
                        if (isTawla) oppTawlas++; 
                        oppPiecesList.push({r, c}); 
                    }

                    score += (isTawla ? 400 : 100) * sign;

                    if (!isTawla && levelNum >= 3) {
        ...
        if (isEndgame && levelNum >= 6) {
            ...
            if (myTawlas > 0 && oppPieces === 0) score += 10000; 
            if (oppTawlas > 0 && myPieces === 0) score -= 10000;
        }
    ...
    quiescence(board, alpha, beta, isMaximizing, currentTurn, aiColor, pieceDirection, levelNum, depthLimit, startTime, maxTime) {
        ...
        captures.sort((a, b) => {
            ...
            const aPromo = (a[a.length - 1].toR === promoRow && aPiece && !aPiece.includes('tawla')) ? 1 : 0;
            const bPromo = (b[b.length - 1].toR === promoRow && bPiece && !bPiece.includes('tawla')) ? 1 : 0;
    ...
    async getBestMoveAsync(virtualBoard, levelStr, aiColor, pieceDirection) {
        ...
        function getMoveScore(move, ttMove, pvMoveLocal, ply, currentTurn, board) {
            ...
            const promoRow = (pieceDirection[currentTurn] === 1 ? 7 : 0);
            const isPromo = (move[move.length - 1].toR === promoRow && piece && !piece.includes('tawla'));
        ...
        async function minimax(board, nominalDepth, searchDepth, isMaximizing, alpha, beta, currentTurn, currentMovesNoProg, currentZobrist, isRoot = false, extensions = 0, ply = 0) {
                ...
                if (pieceCount > 6) {
                    ...
                    if (isMaximizing) {
                        ...
                        if (nullRes.score >= beta) {
                            return { score: beta, timeout: false, pv: [] }; 
                        }
                    } else {
                        ...
                        if (nullRes.score <= alpha) {
                            return { score: alpha, timeout: false, pv: [] }; 
                        }
                    }
                }
            }
            ...
            if (isMaximizing) {
                ...
                for (let i = 0; i < possibleMoves.length; i++) {
                    ...
                    const piece = board[move[0].fromR][move[0].fromC];
                    const promoRow = (pieceDirection[currentTurn] === 1) ? 7 : 0;
                    const isPromo = (lastStep.toR === promoRow && piece && !piece.includes('tawla'));
