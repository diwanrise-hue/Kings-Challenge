/**
 * maint.js
 * المنسق العام لمشروع الطاولة (Orchestrator).
 * تم التحديث: ربط الملفات الجديدة بحرف t وتغيير كل ما يخص الدامة للطاولة.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ui.onClick('login-guest-btn', () => {
        ...
        try { 
            localStorage.setItem('tawla_guest_expiry', Date.now() + (30 * 24 * 60 * 60 * 1000)); 
            localStorage.setItem('hub_user_profile', JSON.stringify(gameState.userProfile)); 
        } catch (e) { }
    ...
    ui.onClick('login-submit-btn', () => {
        ...
        gameState.userProfile = { 
            ...gameState.userProfile, 
            name, 
            id: "TAWLA-" + Math.random().toString(36).substring(2, 8).toUpperCase(), 
            authToken: authToken, 
        ...
        try { 
            localStorage.setItem('hub_user_profile', JSON.stringify(gameState.userProfile)); 
            localStorage.removeItem('tawla_guest_expiry'); 
        } catch (e) { }
